...
export class AuthService {
  ...
  fetchCurrentUser() {
    if (this.currentUser) {
      this.http.get<{ operator: UserData }>(
        `${environment.serverUrl}api/operators/my_account/${this.currentUser.localId}`,
      )
        .subscribe(
          resData => {
            const user = new User(
  ...
  viewReservation(id: string) {
    ...
    if (newReservations.length !== this._currentUser.newReservations.length) {
      this.http.post(
        `${environment.serverUrl}api/operators/${this._currentUser.localId}/view_reservation`,
        {
          reservationId: id
        }
      )
